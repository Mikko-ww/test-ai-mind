...
- 任务运行态（`status`、`issue`、`pr`）统一记录在 `<!-- agent-state:json -->`，不写入 plan YAML。
...
- `Agent-PR-Type` 或 `Agent-Issue-Type`（实体类型）
- `Agent-Phase-Name` 或 `Agent-Task-Key`（按类型互斥）
...
- `agent-plan-validation.yml`：在 plan PR 更新时执行 YAML 契约校验
...
4. 确认 plan PR 通过 `agent-plan-validation.yml` 校验。
5. 系统自动创建任务 issue，并开始派发任务给 Copilot。
6. 按风险等级规则审查并合并任务 PR。
...
- 确认 PR body 的 marker 块包含 `Agent-PR-Type: spec` 与 `Agent-Phase-Name: spec`
