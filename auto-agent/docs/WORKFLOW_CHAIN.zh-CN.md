...
| `agent-plan-validation.yml` | `pull_request.opened/synchronize` | 对 plan PR 执行严格 YAML 契约校验 | `validate-plan.js`, `lib/plan-contract.js` |
...
1. plan PR 在 `opened/synchronize` 阶段先通过 `agent-plan-validation.yml` 校验。
2. plan PR merged 后触发 `handle-plan-merge.js`。
3. `handle-plan-merge.js` dispatch `agent-task-creator.yml`。
4. `create-tasks.js` 从 `plans/issue-<id>.yaml` 生成 task issues。
5. 任务运行态（`status`/`issue`/`pr`）写入 Epic 的 `agent-state` 评论，不回写 plan YAML。
6. `agent-task-creator.yml` 随后 dispatch `agent-task-dispatcher.yml`。
7. `dispatch-task.js` 选择可执行任务并分配给 Copilot。
8. task PR `opened/synchronize` 更新运行态为 `in-review`，`closed+merged` 更新为 `done` 并触发下一轮派发。

对应文件：

- `cli/src/templates/scripts/handle-plan-merge.js`
- `cli/src/templates/workflows/agent-plan-validation.yml`
- `cli/src/templates/scripts/validate-plan.js`
