...
- 唯一路由标识：`Agent-Parent-Issue` + `Agent-Task-Id`
...
| `handle-phase-issue-closed.yml` | `issues.closed` 且 `agent:phase:*` | phase issue 手工关闭补偿提示 | 内嵌 `github-script` |
...
- phase PR：`Agent-Task-Id: spec` 或 `Agent-Task-Id: plan`
- task PR：`Agent-Task-Id: task-<n>`（支持连字符）

消费位置：

- `cli/src/templates/scripts/lib/utils.js` (`parseMarkers`)
- `cli/src/templates/scripts/identify-pr-type.js`
