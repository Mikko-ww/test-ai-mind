...
以下标记用于路由与关联（写在 PR/Issue body 的 marker block 内）：

- `Agent-Schema-Version: 2`
- `Agent-Parent-Issue: <number>`
- `Agent-PR-Type: spec|plan|task`（PR）
- `Agent-Issue-Type: phase|task`（Issue）
- `Agent-Phase-Name: spec|plan|execution`（phase）
- `Agent-Task-Key: task-<slug>`（task）
- `Agent-Retry-Count: <number>`（仅 retry phase issue）
...
- `scripts/lib/marker-parser.js` (`parseAgentMetadata`)
...
```bash
grep -R "agent-request\|agent-state:json\|Agent-Schema-Version\|Agent-Parent-Issue\|Agent-PR-Type\|Agent-Issue-Type\|Agent-Phase-Name\|Agent-Task-Key" cli/src/templates
```

并确保：

- `agent-intake.yml` 只检测 `<!-- agent-request -->`
- 路由主判定依赖 `Agent-PR-Type`
- 不再出现已废弃的历史标记
