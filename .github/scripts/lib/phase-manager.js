...
function getCurrentPhase(state) {
  if (!state.phases) {
    // Legacy state - infer from old phase field
    if (state.phase === 'spec-in-progress') return PHASES.REQUIREMENT;
    if (state.phase === 'plan-in-progress') return PHASES.SPEC;
    if (state.phase === 'executing') return PHASES.EXECUTION;
    return PHASES.REQUIREMENT;
  }
  
  return state.current_phase || PHASES.REQUIREMENT;
}
...
function canStartPhase(state, phase) {
  if (!state.phases) {
    return false; // Legacy state doesn't support phases
  }
